...
import io.kodokojo.ha.config.properties.provider.PropertyValueProvider;
...
public class PropertyResolver {
    ...
    private final Map<String, List<PropertyChangeCallback>> changeCallbacks = new HashMap<>();
    ...
    private class InternalInvoker implements InvocationHandler {
        ...
        public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
            if ("updateProperty".equals(method.getName())) {
                String key = (String) args[0];
                Object newValue =  args[1];
                List<PropertyChangeCallback> propertyChangeCallbacks = changeCallbacks.get(key);
                if (propertyChangeCallbacks != null) {
                    propertyChangeCallbacks.stream().forEach(callback -> callback.changePropertyValue(key, newValue));
                }
                return null;
            } else {
                Key keyAnnotation = method.getAnnotation(Key.class);
                if (keyAnnotation == null) {
                    PropertyChangeRegister propertyChangeRegister = method.getAnnotation(PropertyChangeRegister.class);
                    if (propertyChangeRegister == null) {
                        return method.invoke(proxy, args);
                    } else if (args[0] instanceof PropertyChangeCallback) {
                        PropertyChangeCallback propertyChangeCallback = (PropertyChangeCallback) args[0];
                        List<PropertyChangeCallback> propertyChangeCallbacks = changeCallbacks.get(propertyChangeRegister.value());
                        if (propertyChangeCallbacks == null) {
                            propertyChangeCallbacks = new ArrayList<>();
                        }
                        propertyChangeCallbacks.add(propertyChangeCallback);
                        changeCallbacks.put(propertyChangeRegister.value(), propertyChangeCallbacks);
                        return null;
                    }
                }
